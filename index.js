...
// set audio for alarm
const audio = new Audio('otnickawhereareyou128kringtone-58414.mp3');
audio.loop = true;
...
function snooze() {
    if (a_min != '' || a_hour != ''){
        var snoozMinutes = 03;
        if  (Number(_a_mins) < 50)  {
            snoozMinutes += Number(a_min);
            a_min = String(snoozMinutes);
            a_hour = a_hour;

        } else if (Number(_a_min) >= 50) {
            snoozMinutes = (Number(a_min)+snoozMinutes) - 60;
            if (snoozMinutes === 0 ){
                a_min = '00';            
            }else {
                a_min = '0' + String(snoozMinutes);
            }

            a_hour = Number(a_hour) +1;
            String(a_hour);
        }

        document.getElementById("alarm").innerHTML = 'Alarm: ' + a_hour + ':' + a_min;
